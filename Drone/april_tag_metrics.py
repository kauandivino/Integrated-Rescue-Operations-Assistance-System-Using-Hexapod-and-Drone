...
# Setting the width (w) and height (h) size of the image
w, h = 360, 240

# Initializing the April Tags detector
...
# Function to detect April Tags in the image
def detect_april_tag(frame, detector):
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    results = detector.detect(gray)
    if results:
        r = results[0]  # Considering only the first detected tag
...
# Function to determine action based on quadrant position
...
# Main loop for continuous detection and tracking
while True:
    try:
        ...
        if april_tag_info is not None:
            ...
            # Simulating the real position (e.g. center of the image)
